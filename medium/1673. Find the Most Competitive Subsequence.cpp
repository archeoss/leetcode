/*
  ...
  resulting sequence obtained by erasing some (possibly zero) elements from the
  array. We define that a subsequence a is more competitive than a subsequence b
  ...
  example, [1,3,4] is more competitive than [1,3,5] because the first position
...
public:
  vector<int> mostCompetitive(vector<int> &nums, int k) {
    ...
    for (int i = 0; i < len; i++) {
      int num = nums[i];
      while (n && res.back() > num && k - n < len - i) {
        res.pop_back();
        n--;
      }
